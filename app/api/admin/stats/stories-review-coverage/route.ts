...
export async function GET(request: NextRequest) {
    ...
    if (adminCheckResponse) {
        console.warn("WARNING: Admin check skipped for /api/admin/stats/stories-review-coverage endpoint.");
    ...
    try {
        ...
        if (db) {
            try { await db.close(); } catch (closeError) { console.error("Error closing DB after coverage fetch error:", closeError); }
        }
        return NextResponse.json({ error: "Failed to fetch stories review coverage", details: error.message }, { status: 500 });
    }
}