// app/api/admin/recent-reviews/route.ts
import { NextResponse } from "next/server";
import { openDb } from "@/lib/db";

const periodMap: { [key: string]: string } = {
    '5m': '-5 minutes', '30m': '-30 minutes', '1h': '-1 hour', '3h': '-3 hours',
    '6h': '-6 hours', '24h': '-24 hours', 'all': '-100 years'
};
...
export async function GET(request: Request) {
    ...
    try {
        ...
        return NextResponse.json(
            { error: "Failed to fetch recent reviews", details: error instanceof Error ? error.message : String(error) },
            { status: 500 }
        );
    } finally {
