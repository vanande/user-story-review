  ...
  try {
    ...
    try {
      ...
      for (const [criterionName, ratingStr] of Object.entries(data.evaluations)) {
        ...
        if (!criterionId) { /* ... handle error, rollback ... */
          await db.exec("ROLLBACK TRANSACTION");
          return NextResponse.json({ error: `Unknown criterion: ${criterionName}` }, { status: 400 });
        }
        await db.run(`INSERT INTO criterion_evaluations (review_id, criterion_id, rating) VALUES (?, ?, ?)`, [reviewId, criterionId, ratingInt]);
      }
      // ---------------------------------

      await db.exec("COMMIT TRANSACTION");
      console.log("SQLite transaction committed successfully");
      return NextResponse.json({ success: true, message: "Feedback stored successfully", reviewId }, { status: 201 });
    ...
    return NextResponse.json( { error: "Failed to process feedback", details: error instanceof Error ? error.message : String(error) }, { status: 500 });
  } finally {
    if (db) { await db.close(); console.log("SQLite database connection closed."); }
  }
}