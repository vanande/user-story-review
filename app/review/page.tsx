...
export default function ReviewPage() {
  ...
  const [relatedEpicStoriesFromAPI, setRelatedEpicStoriesFromAPI] = useState<UserStory[]>([]);
  const [isLoadingRelatedStories, setIsLoadingRelatedStories] = useState(false);
  ...
  const safeStoryIndex = userStories.length > 0 ? Math.min(currentStoryIndex, userStories.length - 1) : 0;
  const currentStory = userStories.length > 0 ? userStories[safeStoryIndex] : undefined;

  useEffect(() => {
    if (currentStory && currentStory.id && currentStory.datasetId) {
      const fetchRelatedStories = async () => {
        setIsLoadingRelatedStories(true);
        setRelatedEpicStoriesFromAPI([]);
        let url = `/api/related-stories?currentStoryId=${currentStory.id}&datasetId=${currentStory.datasetId}`;
        ...
        if (currentStoryHasEpicId) {
          url += `&epicId=${encodeURIComponent(currentStory.epicId!)}`;
        } else if (currentStory.epic_name && currentStory.source_key) {
          url += `&epicName=${encodeURIComponent(currentStory.epic_name)}&sourceKey=${encodeURIComponent(currentStory.source_key)}`;
        } else {
          setIsLoadingRelatedStories(false);
          return;
        }
        ...
        try {
          ...
          if (!response.ok) {
            console.error("Failed to fetch related stories", response.status);
            setRelatedEpicStoriesFromAPI([]);
            return;
          }
          const data = await response.json();
          if (data.success && Array.isArray(data.stories)) {
            setRelatedEpicStoriesFromAPI(data.stories);
          } else {
            setRelatedEpicStoriesFromAPI([]);
          }
        } catch (err) {
    ...
    return (
      <div className="container mx-auto max-w-4xl p-4">
        <Alert variant={error ? "destructive" : "default"}>
          ...
          <AlertTitle>{error ? "Erreur" : "Aucune story disponible"}</AlertTitle>
          <AlertDescription>
            {error || "Il n'y a actuellement aucune user story à annoter. Merci de réessayer plus tard ou de contacter un administrateur."}
          </AlertDescription>
        </Alert>
        <div className="mt-4 space-x-2">
            {error && <Button onClick={() => window.location.reload()}>Rafraîchir la page</Button>}
            <Button onClick={() => router.push("/dashboard")} variant={error ? "outline" : "default"}>
            ...
            <CardContent>
              <div className="flex flex-col items-center">
                <div className="w-full mb-6 text-center">
                  ...
                  {currentPrinciple.id === "independent" && (
                    <div className="mb-6 text-left">
                      <Accordion type="single" collapsible className="w-full">
                        <AccordionItem value="related-stories" className="border rounded-md px-3">
                          <AccordionTrigger className="text-sm font-medium hover:no-underline py-2">
                            {isLoadingRelatedStories
                              ? "Chargement des stories liées..."
                              : relatedEpicStoriesFromAPI.length > 0
                              ? `Voir ${relatedEpicStoriesFromAPI.length} autre(s) story(s) dans l'épique "${currentStory.epic_name || 'N/A'}"`
                              : "Aucune autre story trouvée dans cette épique"}
                          </AccordionTrigger>
                          {!isLoadingRelatedStories && relatedEpicStoriesFromAPI.length > 0 && (
                            <AccordionContent className="pt-2 pb-3">
                              <ul className="list-disc space-y-1.5 pl-5 text-xs text-muted-foreground max-h-48 overflow-y-auto">
                                {relatedEpicStoriesFromAPI.map((story) => (
                                  <li key={story.id} className="break-words">
                                    ...
                                    {/* <span className="font-semibold text-foreground">#{story.id} ({story.source_key || 'N/A'}):</span> {story.description || "(Pas de description)"} */}
                                  </li>
                                ))}
                              </ul>
                            </AccordionContent>
